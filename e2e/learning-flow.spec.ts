...
test("업데이트 내역은 날짜와 첫 공개 내용을 보여 준다", async ({ page }) => {
  ...
  await expect(page.getByRole("dialog", { name: "업데이트 내역" })).toContainText("v1.3.0");
...
test("고정 9개 미션을 마치면 점수 없는 배송 정리표가 열린다", async ({ page }) => {
  ...
  await page.getByLabel("3묶음 0낱개", { exact: true }).check();
  await expect(page.getByText("30개를 정확히 나타냈어요.")).toBeVisible();
